...
from DanQ_model import precision_metric, recall_metric, dice_score, DataGenerator
...
for i in range(num_classes):
    ...
    class_recall = recall_score(test_y[:,i],y_pred_binary[:,i])
    class_f1 = f1_score(test_y[:, i], y_pred_binary[:, i])
    df[labels[i]] = {"Accuracy":class_accuracy,"Precision":class_precision,"Recall":class_recall,"F1-Score":class_f1}
df =pd.DataFrame(df)
df.to_csv('test_osa.csv')
